...
#define JAN_1970     0x83AA7E80             ///< 1900 ~ 1970 年之间的时间 秒数 Time between 1900 ~ 1970 seconds
#define NS100_1970   116444736000000000LL   ///< 1601 ~ 1970 年之间的时间 百纳秒数 Time between 1601 ~ 1970 100 100 nanoseconds

/**
 * @enum  em_ntp_mode_t
 * @brief NTP工作模式的相关枚举值。 The relevant enumeration value for the NTP operating mode
 */
typedef enum em_ntp_mode_t
{
    ntp_mode_unknow     = 0,  ///< 未定义 Undefined
    ntp_mode_initiative = 1,  ///< 主动对等体模式 Active peer mode
    ntp_mode_passive    = 2,  ///< 被动对等体模式 Passive peer mode
    ntp_mode_client     = 3,  ///< 客户端模式 Client mode
    ntp_mode_server     = 4,  ///< 服务器模式 Server mode
    ntp_mode_broadcast  = 5,  ///< 广播模式或组播模式 Broadcast mode or multicast mode
    ntp_mode_control    = 6,  ///< 报文为 NTP 控制报文 The message is an NTP control packet
    ntp_mode_reserved   = 7,  ///< 预留给内部使用 Reserved for internal use
} em_ntp_mode_t;

/**
 * @struct x_ntp_packet_t
 * @brief  NTP 报文格式。Message format.
 */
typedef struct x_ntp_packet_t
{
    x_uchar_t         xct_li_ver_mode;      ///< 2 bits，飞跃指示器；3 bits，版本号；3 bits，NTP工作模式（参看 em_ntp_mode_t 相关枚举值）2 bits, leap indicator; 3 bits, version number; 3 bits, NTP operating mode (see em_ntp_mode_t Related enumeration values)
    x_uchar_t         xct_stratum    ;      ///< 系统时钟的层数，取值范围为1~16，它定义了时钟的准确度。层数为1的时钟准确度最高，准确度从1到16依次递减，层数为16的时钟处于未同步状态，不能作为参考时钟 系统时钟的层数，取值范围为1~16，它定义了时钟的准确度。层数为1的时钟准确度最高，准确度从1到16依次递减，层数为16的时钟处于未同步状态，不能作为参考时钟
    x_uchar_t         xct_poll       ;      ///< 轮询时间，即两个连续NTP报文之间的时间间隔 Polling time, which is the time interval between two consecutive NTP packets
    x_uchar_t         xct_percision  ;      ///< 系统时钟的精度  The accuracy of the system clock

    x_uint32_t        xut_root_delay     ;  ///< 本地到主参考时钟源的往返时间 The round-trip time locally to the primary reference source
    x_uint32_t        xut_root_dispersion;  ///< 系统时钟相对于主参考时钟的最大误差 The maximum error of the system clock relative to the primary reference clock
    x_uint32_t        xut_ref_indentifier;  ///< 参考时钟源的标识 The identification of the reference clock source

    x_ntp_timestamp_t xtmst_reference;      ///< 系统时钟最后一次被设定或更新的时间（应答完成后，用于存储 T1） The last time the system clock was set or updated (used to store T1 after the answer is completed)
    x_ntp_timestamp_t xtmst_originate;      ///< NTP请求报文离开发送端时发送端的本地时间（应答完成后，用于存储 T4）  The local time of the sender when the NTP request packet leaves the sender (after the reply is completed, it is used to store T4)
    x_ntp_timestamp_t xtmst_receive  ;      ///< NTP请求报文到达接收端时接收端的本地时间（应答完成后，用于存储 T2） The local time of the receiver when the NTP request packet arrives at the receiver (after the reply is completed, it is used to store T2)
    x_ntp_timestamp_t xtmst_transmit ;      ///< NTP应答报文离开应答者时应答者的本地时间（应答完成后，用于存储 T3） The local time of the responder when the NTP reply message leaves the responder (used to store T3 after the reply is complete.)
} x_ntp_packet_t;
 ...
 *
 * @param [in ] xszt_vptr : 判断的字符串。 The string of judgments
 * @param [out] xut_value : 若入参不为 X_NULL，则操作成功时，返回对应的 IP 地址值。 If the input parameter is not X_NULL, the corresponding IP address value is returned when the operation is successful.
 *
 * @return x_bool_t
 *         - 成功，返回 X_TRUE；
 *         - 失败，返回 X_FALSE。
 ...
 *
 * @param [in ] xszt_dname : 指定的域名（格式如：www.163.com）。The specified domain name (in the format: www.163.com).
 * @param [in ] xit_family : 期待返回的套接口地址结构的类型。The type of socket address structure expected to be returned
 * @param [out] xvec_host  : 操作成功返回的地址列表。The list of addresses returned for a successful operation.
 *
 * @return x_int32_t
 *         - 成功，返回 0；
 *         - 失败，返回 错误码。
 ...
 * @param [in ] xut_tmout : 超时时间（单位 毫秒）。Timeout (in milliseconds)
 * @param [out] xit_tmlst : 操作成功返回的相关计算所需的时间戳（T1、T2、T3、T4）。The timestamp required for the computation returned by the operation successfully (T1, T2, T3, T4).
 *
 * @return x_int32_t
 *         - 成功，返回 0；
 *         - 失败，返回 错误码。
...
static x_int32_t ntp_get_time_values(x_cstring_t xszt_host, x_uint16_t xut_port, x_uint32_t xut_tmout, x_int64_t xit_tmlst[4])
{
    ...
    do 
    {
        ...
        // 服务端主机地址  The server host address
        ...
        // 初始化请求数据包  Initialize the request packet
 ...
 * @param [in ] xut_port  : NTP 服务器的 端口号（可取默认的端口号 NTP_PORT : 123）。  Port number of the NTP server (default port number NTP_PORT : 123)
 * @param [in ] xut_tmout : 网络请求的超时时间（单位为毫秒）。 Timeout for network requests in milliseconds
...
/**********************************************************/
/**
 * @brief 向 NTP 服务器发送 NTP 请求，获取服务器时间戳，用于测试。 Send an NTP request to the NTP server to get the server timestamp for testing.
 * 
 * @param [in ] xszt_host : NTP 服务器的 IP（四段式 IP 地址） 或 域名（如 3.cn.pool.ntp.org）。The IP (four-segment IP address) or domain name (such as 3.cn.pool.ntp.org) of the NTP server.
 * @param [in ] xut_port  : NTP 服务器的 端口号（可取默认的端口号 NTP_PORT : 123）。The port number of the NTP server (whichever is the default port number NTP_PORT : 123).
 * @param [in ] xut_tmout : 网络请求的超时时间（单位为毫秒）。 网络请求的超时时间（单位为毫秒）。The timeout period for network requests in milliseconds.
 * @param [out] xut_timev : 操作成功返回的应答时间值（以 100纳秒 为单位，1970年1月1日到现在的时间）。The answer time value returned by the successful operation in 100 nanoseconds from January 1, 1970 to the present.
 * 
 * @return x_int32_t
 *         - 成功，返回 0；
...
x_int32_t ntp_get_time_test(x_cstring_t xszt_host, x_uint16_t xut_port, x_uint32_t xut_tmout, x_uint64_t * xut_timev)
{
    ...
    //======================================
    // 获取 IP 地址列表 Get a list of IP addresses
