...
int64_t GetPledgeTimeByAddr(const std::string &addr, global::ca::StakeType stakeType, const std::shared_ptr<GetSDKAck> &SDKAckMsg);

// //Obtain the number of blocks per unit time below 500 according to the height and unit time
// int GetBlockNumInUnitTime(uint64_t block_height, uint64_t unit_time = 5 * 60 * 1000000);
...
int CalcBonusValue(uint64_t &cur_time, const std::string &bonusAddr,std::map<std::string, uint64_t> & vlaues,const std::shared_ptr<GetSDKAck> &SDKAckMsg);
int CalcBonusValue();
// // //Calculate annual rate of return 
// // int CalcInflationRate();
